...
export const useRunsStore = create<RunsStore>((set, get) => ({
  ...
  fetch: async () => {
    ...
    try {
      const res = await fetch(`/api/runs?limit=${PAGE}&offset=0`);
      if (!res.ok) throw new Error("Failed to load history");
      const data = await res.json() as RunResponse[];
      set({ runs: data, hasMore: data.length === PAGE, _offset: data.length, initialized: true, stale: false });
    } catch (err) {
  ...
  fetchMore: async () => {
    ...
    try {
      const res = await fetch(`/api/runs?limit=${PAGE}&offset=${_offset}`);
      if (!res.ok) throw new Error("Failed to load history");
      const data = await res.json() as RunResponse[];
      set((s) => ({
        runs:    [...s.runs, ...data],
